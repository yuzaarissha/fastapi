...
async def create_note(data: NoteCreate, owner: User,
...
async def get_note(note_id: int, owner: User,
                   session: AsyncSession) -> Note:
...
async def list_notes(owner: User, session: AsyncSession) -> list[Note]:
    res = await session.execute(select(Note).where(Note.owner_id == owner.id))
    return res.scalars().all()

async def update_note(note_id: int, data: NoteUpdate, owner: User,
                      session: AsyncSession) -> Note:
...
async def delete_note(note_id: int, owner: User,
