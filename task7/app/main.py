...
from .crud import (
    register_user, login_user, list_users,
...
app = FastAPI(title="Notes API with Ownership")
...
async def me(curr: User = Depends(get_current_user)): return curr

@app.get("/admin/users", response_model=list[UserOut],
         dependencies=[Depends(require_role("admin"))])
async def admin_users(s=Depends(get_session)): return await list_users(s)
...
async def my_notes(curr: User = Depends(get_current_user),
                   s=Depends(get_session)):
    return await list_notes(curr, s)
...
async def edit_note(note_id: int, n: NoteUpdate,
                    curr: User = Depends(get_current_user),
                    s=Depends(get_session)):
