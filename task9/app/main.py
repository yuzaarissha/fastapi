...
async def add_note(n: NoteCreate, curr: User = Depends(get_current_user),
                   s=Depends(get_session)):
    return await create_note(n, curr, s)


@app.get("/notes", response_model=list[NoteOut])
async def my_notes(skip: int = Query(0, ge=0),
...
async def read_note(note_id: int, curr: User = Depends(get_current_user),
                    s=Depends(get_session)):
...
async def remove_note(note_id: int, curr: User = Depends(get_current_user),
                      s=Depends(get_session)):
